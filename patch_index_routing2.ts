...
// 1. Replace the body destructuring
content = content.replace(
  'const { message, sessionId } = body;',
  'const { action, word, note, dictionary_data, message, sessionId } = body;'
);

// 2. We want to insert the routing block right after body destructuring, before the `if (!message)` check.
const targetInsertAfter = `const { action, word, note, dictionary_data, message, sessionId } = body;`;

const authCodeToMove = `
...
content = content.replace(targetInsertAfter, targetInsertAfter + authCodeToMove);

// 3. Remove original auth check further down
const authInitTarget = `      // Supabase Client Initialization for Persistence
      const supabaseUrl = Deno.env.get('SUPABASE_URL') || '';
      const supabaseAnonKey = Deno.env.get('SUPABASE_ANON_KEY') || '';
...
content = content.replace(authInitTarget, '// Auth logic extracted to top of function for action routing');
